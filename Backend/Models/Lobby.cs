...
public abstract class Lobby<LobbyPlayerType, LobbyOptionsType> : ILobbyCollectable, ILobbyPlace where LobbyPlayerType : IPlayer where LobbyOptionsType : ILobbyOptions
{
    public readonly LobbyOptionsType LobbyOptions;
    ...
    /// Connected players
    /// </summary>
    public readonly List<LobbyPlayerType> Players = new List<LobbyPlayerType>();
    ...
    /// </summary>
    public DateTime LastLobbySnapshot { get; private set; }

    /// <summary>
    /// Dictionary of places where key is userId and value is placeId
    /// </summary>
    public Dictionary<int, int> LobbyPlaces { get; } = new Dictionary<int, int>();

    public Lobby(uint lobbyCreatorId, uint lobbyId, ILobbyOptions options)
    {
        LobbyId = lobbyId;
        LobbyCreatorId = lobbyCreatorId;    
        LobbyOptions = (LobbyOptionsType)options;
    ...
    public bool ChangeMaxPlayers(int state)
    {
        ...
        bool validate = state <= LobbyOptions.MaxPlayersLimit && state >= LobbyOptions.MinPlayersLimit;
    ...
    public bool ChoosePlace(int userId, int placeId)
    {
        ...
        if (placeId >= LobbyOptions.MaxPlayersLimit)
        {
            return false;
        }
